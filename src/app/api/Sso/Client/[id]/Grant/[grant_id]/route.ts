...
export async function GET(
  req: Request,
  { params }: { params: Promise<{ id: string; grant_id: string }> }
) {
  ...
  try {
    const res = await fetch(`${apiBaseUrl}/api/v2/Account/Client/${id}/Grant/${grant_id}`, {
      method: "GET",
      headers: {
...
export async function PATCH(
  ...
  const { id, grant_id } = await params;
  try {
    const ref = await fetch(`${apiBaseUrl}/api/v2/Account/Client/${id}/Grant/${grant_id}`, {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${session}`,
        "Content-Type": "application/json",
