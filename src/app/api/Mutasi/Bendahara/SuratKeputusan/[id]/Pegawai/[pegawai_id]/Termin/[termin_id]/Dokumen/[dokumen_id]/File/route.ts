...
export async function GET(
  ...
  const { id, dokumen_id, pegawai_id, termin_id } = await params;

  try {
    const res = await fetch(
      `${apiBaseUrl}/api/v2/Bendahara/SuratKeputusan/${id}/Pegawai/${pegawai_id}/Termin/${termin_id}/Dokumen/${dokumen_id}/File`,
      {
        method: "GET",
        headers: {
