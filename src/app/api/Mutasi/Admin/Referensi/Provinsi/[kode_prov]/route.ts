...
export async function GET(req: Request, { params }: { params: Promise<{ kode_prov: string }> }) {
  ...
  try {
    const res = await fetch(`${apiBaseUrl}/api/v2/Admin/Referensi/Provinsi/${kode_prov}`, {
      method: "GET",
      headers: {
...
export async function PATCH(req: Request, { params }: { params: Promise<{ kode_prov: string }> }) {
  const session = (await cookies()).get(`${process.env.APP_NAME}.session`)?.value;
  if (!session) {
    return NextResponse.json({ message: "Unauthorized" }, { status: 500 });
  }
  ...
  try {
    const ref = await fetch(`${apiBaseUrl}/api/v2/Admin/Referensi/Provinsi/${kode_prov}`, {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${session}`,
        "Content-Type": "application/json",
