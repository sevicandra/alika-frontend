...
export async function GET(
  ...
  try {
    const suratKeputusan = await fetch(
      `${apiBaseUrl}/api/v2/SDM/SuratKeputusan/${id}/Pegawai/${pegawai_id}/Termin?${searchParamsString.toString()}`,
      {
        method: "GET",
        headers: {
...
export async function POST(
  ...
  try {
    const suratKeputusan = await fetch(
      `${apiBaseUrl}/api/v2/SDM/SuratKeputusan/${id}/Pegawai/${pegawai_id}/Termin`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
