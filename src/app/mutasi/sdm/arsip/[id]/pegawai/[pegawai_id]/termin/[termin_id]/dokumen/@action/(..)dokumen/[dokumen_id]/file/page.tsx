...
export default function Page({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const file = await fetch(
          `/api/Mutasi/SDM/SuratKeputusan/${id}/Pegawai/${pegawai_id}/Termin/${termin_id}/Dokumen/${dokumen_id}/File`,
          {
            method: "GET",
            headers: {
        ...
        const { error, data } = await file.json();
        if (!file.ok) {
          throw new Error(
            error.message
    ...
    fetchData();
  }, [addNotification, id, pegawai_id, termin_id, dokumen_id]);
