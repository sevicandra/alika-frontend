...
export default function Page() {
  ...
  async function submitForm(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      setLoading(true);
      const res = await fetch(`/api/Mutasi/Admin/Referensi/Darat`, {
        headers: {
  ...
  return (
    ...
    >
      <div className="grid grid-cols-2 gap-x-8 gap-y-6">
        ...
        <div className="form-control col-span-2">
          <label className="label">
            <span className="label-text font-semibold">Pulau</span>
          </label>
          <div className="relative">
            <span className="absolute top-1/2 left-3 z-10 -translate-y-1/2 text-base-content/50">
              <Icon icon="ChevronsUpDown" height={20} />
            ...
            <select
              name="pulau"
              className={`select-bordered select w-full pl-10 ${getValidationError("pulau") ? "select-error" : ""}`}
              required
              value={input.pulau || ""}
              onChange={(e) => setInput({ ...input, pulau: e.target.value })}
            >
        ...
        <div className="form-control col-span-2">
          <label className="label">
            <span className="label-text font-semibold">Jarak</span>
          </label>
          <div className="relative">
            <span className="absolute top-1/2 left-3 z-10 -translate-y-1/2 text-base-content/50">
              <Icon icon="FileText" height={20} />
            ...
            <input
              type="text"
              name="jarak"
              className={`input-bordered input w-full pl-10 ${getValidationError("jarak") ? "input-error" : ""}`}
              value={input.jarak ? input.jarak.toLocaleString("id-ID") : ""}
              onChange={(e) => {
                const rawValue = e.target.value.replace(/[^\d]/g, "");
                const numericValue = Number(rawValue);
                if (!isNaN(numericValue)) {
                  setInput({ ...input, jarak: numericValue });
                }
              }}
              required
            />
