  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setLoading(true);
        const res = await fetch(`/api/Mutasi/Admin/Referensi/UangHarian?${searchParams}`, {
          method: "GET",
        });
        if (!res.ok) {
        ...
        addNotification({
          title: "Fetch Referensi Uang Harian",
    ...
    >
      <div className="relative grid grid-rows-[1fr_auto] overflow-hidden">
        ...
        <div className="overflow-y-auto">
          <DataTable
            columns={["No.", "Provinsi", "Tarif", ""]}
            data={data}
            renderRow={(row, index) => (
              <tr key={index}>
                <td className="p-4">{index + 1}</td>
                <td className="p-4">{row.provinsi}</td>
                <td className="p-4">{row.tarif.toLocaleString("id-ID")}</td>
                <td className="p-4">
                  <div className="flex gap-1">
                    <div className="tooltip" data-tip="edit">
                      <Link href={`/mutasi/admin/referensi/uang-harian/${row.id}/edit`}>
                    ...
                    <div className="tooltip" data-tip="hapus">
                      <Link href={`/mutasi/admin/referensi/uang-harian/${row.id}/hapus`}>
