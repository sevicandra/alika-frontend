  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setLoading(true);
        const res = await fetch(`/api/Mutasi/Admin/Referensi/Barang?${searchParams}`, {
          method: "GET",
        });
        if (!res.ok) {
        ...
        addNotification({
          title: "Fetch Referensi Volume Barang",
    ...
    >
      <div className="relative grid grid-rows-[1fr_auto] overflow-hidden">
        ...
        <div className="overflow-y-auto">
          <DataTable
            columns={["No.", "Golongan", "Status Tanggungan", "Volume", ""]}
            data={data}
            renderRow={(row, index) => (
              <tr key={index}>
                <td className="p-4">{index + 1}</td>
                <td className="p-4">{row.golongan}</td>
                <td className="p-4">{snackToTitleCase(row.status)}</td>
                <td className="p-4">{row.volume}</td>
                <td className="p-4">
                  <div className="flex gap-1">
                    <div className="tooltip" data-tip="edit">
                      <Link href={`/mutasi/admin/referensi/barang/${row.id}/edit`}>
                    ...
                    <div className="tooltip" data-tip="hapus">
                      <Link href={`/mutasi/admin/referensi/barang/${row.id}/hapus`}>
