...
import { usePegawaiDetail } from "@/context/mutasi/bendahara";
...
export default function Page({
  ...
  const { error, loading, setError, setLoading } = usePegawaiDetail();
  ...
  useEffect(() => {
    const fetchPegawai = async () => {
      ...
      try {
        const res = await fetch(
          `/api/Mutasi/Bendahara/SuratKeputusan/${id}/Pegawai/${pegawai_id}/Keluarga?${searchParams}`,
          {
            method: "GET",
          },
        );
    ...
    fetchPegawai();
  }, [addNotification, id, pegawai_id, setError, setLoading]);
                ...
                <tr key={index}>
                  ...
                  <td className="px-4 py-2">
                    <div className="tooltip" data-tip="termin">
                      {row.file && (
                        <Link
                          href={`/mutasi/bendahara/sk/${id}/pegawai/${pegawai_id}/keluarga/${row.id}/file`}
                        >
